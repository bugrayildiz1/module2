...
public class Zipper {
    ...
    */
    public static String zip2(String s1, String s2) throws TooFewArgumentsException,
            ArgumentLengthsDifferException {
        if (s1 == null || s2 == null) {
            throw new TooFewArgumentsException("must pass two command line arguments");
        } else if (s1.length() != s2.length()) {
            throw new ArgumentLengthsDifferException("length of command line arguments differs ("
                    + s1.length() + ", " + s2.length() + ")");
        }
