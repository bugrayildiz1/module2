...
public class Game {
     ...
     * Asks after each ended game if the user want to continue. Continues until the user does not
    ...
    /**
     * Prints a question which can be answered by yes (true) or no (false). After prompting the
     * question on standard out, this method reads a String from standard in and compares it to the
     * parameters for yes and no. If the user inputs a different value, the prompt is repeated and
     * te method reads input again.
     * 
     ...
     * First the (still empty) board is shown. Then the game is played until it is over. Players can
     * make a move one after the other. After each move, the changed game situation is printed.
     */
    private void play() {
        ...
        while (!board.gameOver()) {
            ...
            // Let the current player make his move, then print the updated board.
    ...
    private void printResult() {
        if (board.hasWinner()) {
            ...
            System.out.println("Speler " + winner.getName() + " (" + winner.getMark().toString()
                    + ") has won!");
        } else {
